...
esp_err_t qmc5883l_write_byte(uint8_t reg_addr, uint8_t data,
  ...
  ESP_ERROR_CHECK(i2c_master_write_byte(
      cmd, (QMC5883L_ADDRESS << 1) | I2C_MASTER_WRITE, true));
...
esp_err_t qmc5883l_read_bytes(uint8_t reg_addr, uint8_t *data, size_t len,
  ...
  ESP_ERROR_CHECK(i2c_master_start(cmd));
  ESP_ERROR_CHECK(i2c_master_write_byte(
      cmd, (QMC5883L_ADDRESS << 1) | I2C_MASTER_READ, true));
...
void qmc5883l_read_magnetometer(int16_t *x, int16_t *y, int16_t *z) {
  uint8_t data[6];
  if (qmc5883l_read_bytes(QMC5883L_REG_X_LSB, data, 6, I2C_MASTER_PORT) ==
      ESP_OK) {
